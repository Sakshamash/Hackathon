...
const GlassIcons = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-[2em] md:grid-cols-4 overflow-visible ${className || ""
        }`}
    >
      {items.map((item, index) => (
        <button
          ...
          Link="{item.Link}"
          className={`relative bg-transparent outline-none w-[2em] h-[2em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${item.customClass || ""
            }`}
        >
          <span
          ...
          >
            <span className="m-auto w-[1em] h-[1em] flex items-center justify-center" aria-hidden="true">
              {item.icon}
            </span>
          </span>
